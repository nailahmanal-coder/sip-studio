  ...
  return (
    <div className="w-64 flex-shrink-0 bg-white border-l-2 border-[#e0d0bc] flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4" style={{scrollbarWidth:'thin'}}>
        <div className="text-sm font-black text-[#2c1a08] mb-3">✦ Your order</div>
        {rows.length === 0 ? (
      ...
      <div className="p-4 border-t-2 border-[#e0d0bc] bg-[#fde8f0] flex flex-col gap-2 flex-shrink-0">
        <div className="text-[11px] font-black text-[#c0607a]">🌸 Personal note</div>
        ...
        <textarea
          ...
          maxLength={110}
          rows={3}
          className="w-full border-2 border-[#f0b8d0] rounded-lg px-2.5 py-1.5 text-[15px] bg-white text-[#2c1a08] outline-none focus:border-[#c0607a] resize-none leading-snug"
        ...
        <button
          onClick={onClip}
          disabled={!base}
          className="w-full py-3 rounded-xl bg-[#2c1a08] text-[#faf7f2] text-sm font-black transition-all hover:-translate-y-0.5 hover:shadow-lg disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:translate-y-0"
        >
          📌 Pin to message wall
        </button>
      </div>
