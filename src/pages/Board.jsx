...
function OrderCard({ order, isAdmin, onDelete }) {
  ...
  return (
    <div className="bg-white rounded-2xl border-2 border-[#e0d0bc] overflow-hidden transition-all duration-200 hover:-translate-y-1 hover:shadow-xl animate-bounce-in relative">
      {isAdmin && (
        <button
          onClick={() => onDelete(order.id)}
          className="absolute top-2 right-2 z-10 bg-red-500 hover:bg-red-600 text-white text-[10px] font-black px-2 py-1 rounded-full transition-all hover:scale-105"
        >
          🗑 delete
        </button>
      )}
      ...
      <div className="p-3 bg-white">
        ...
        <div className="mt-2 text-[10px] text-[#a09080] font-bold flex items-center gap-1">
          with love from <span className="text-[#5a8a3c] font-black text-[12px]" style={{fontFamily:"'Caveat', cursive"}}>{order.from_name}</span> 🤍
        </div>
      </div>
    </div>
  )
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="px-5 py-3 border-b-2 border-[#e0d0bc] bg-[#f5ede0] flex items-center justify-between flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          {!isAdmin ? (
            <button
              onClick={() => setShowPwBox(!showPwBox)}
              className="text-[10px] text-[#a09080] hover:text-[#2c1a08] font-bold transition-all"
            >
              ⚙️
            </button>
          ) : (
            <button
              ...
              className="text-[10px] bg-red-100 text-red-500 font-black px-2 py-1 rounded-full"
            >
              exit admin
            </button>
          )}
      ...
      {showPwBox && (
        <div className="px-5 py-2 bg-[#2c1a08] flex items-center gap-2 flex-shrink-0">
          <span className="text-white text-[11px] font-bold">Admin password:</span>
          <input
            type="password"
            value={pwInput}
          ...
          <button
            ...
            className="bg-[#5a8a3c] text-white text-[11px] font-black px-3 py-1 rounded-lg"
          >
            Login
          </button>
        </div>
