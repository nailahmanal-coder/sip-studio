...
const SWEETENERS = ['No sugar','🍯 Honey','Vanilla syrup','Brown sugar','Agave','🍁 Maple','💜 Lavender','🌹 Rose','Coconut sugar','Date syrup','Hazelnut','Caramel']
...
function Chip({ label, selected, onClick, color }) {
  return (
    <button
      onClick={onClick}
      className={`px-3 py-1 rounded-full border-2 text-[11px] font-bold transition-all hover:-translate-y-0.5 ${
        selected
          ...
          : 'bg-white border-[#e0d0bc] text-[#6a5040] hover:border-[#7aaa50]'
      }`}
    >
      {label}
    </button>
...
export default function Build({ onClip }) {
  ...
  return (
    <div className="flex h-full overflow-hidden">
        ...
        <div>
            ...
            {BASES.map(b => {
              ...
              return (
                <button key={b.id} onClick={() => { setBase(b.id); setGrade(null); setShots(null) }}
                  className={`border-2 rounded-xl p-2.5 text-center transition-all duration-150 hover:-translate-y-1 hover:shadow-lg relative bg-white ${sel ? `${col.border} ${col.bg} -translate-y-1 shadow-lg` : 'border-[#e0d0bc]'}`}>
          ...
          {isCoffee && (
            <div className="mt-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {['Single (1)','Double (2)','Triple (3)','Quad (4)'].map(s => <Chip key={s} label={s} selected={shots === s} onClick={() => setShots(s)} color="coffee" />)}
              </div>
            </div>
          )}
        </div>
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {MILKS.map(m => (
              <button key={m.id} onClick={() => setMilk(m.id)}
                className={`border-2 rounded-xl p-2 text-center transition-all hover:-translate-y-1 hover:shadow-md ${milk === m.id ? 'border-[#5a8a3c] bg-[#eef8e0] -translate-y-1 shadow-md' : 'border-[#e0d0bc] bg-white'}`}>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <button key={t.id} onClick={() => { setTemp(t.id); if(t.id !== 'cold') setIce(null) }}
                className={`border-2 rounded-xl p-3 text-center transition-all hover:-translate-y-1 hover:shadow-md ${temp === t.id ? `${t.onCls} -translate-y-1 shadow-md` : 'border-[#e0d0bc] bg-white'}`}>
          ...
          <div className="grid grid-cols-4 gap-2">
            {[
              ...
              <button key={s.id} onClick={() => setSize(s.id)}
                className={`border-2 rounded-xl p-2 text-center transition-all hover:-translate-y-1 hover:shadow-md ${size === s.id ? 'border-[#5a8a3c] bg-[#eef8e0] -translate-y-1 shadow-md' : 'border-[#e0d0bc] bg-white'}`}>
          ...
          <div className="grid grid-cols-4 gap-1.5">
            {FOAMS.map(f => (
              <button key={f.id} onClick={() => setFoam(f.id)}
                className={`border-2 rounded-xl p-2 text-center transition-all hover:-translate-y-1 hover:shadow-md ${foam === f.id ? 'border-[#5a8a3c] bg-[#eef8e0] -translate-y-1 shadow-md' : 'border-[#e0d0bc] bg-white'}`}>
          ...
          <div className="grid grid-cols-4 gap-1.5">
            {TOPPINGS.map(t => (
              <button key={t.id} onClick={() => toggleTopping(t.id)}
                className={`border-2 rounded-xl p-2 text-center transition-all hover:-translate-y-1 hover:shadow-md ${toppings.includes(t.id) ? 'border-[#7aaa50] bg-[#eef8e0] scale-105 shadow-md' : 'border-[#e0d0bc] bg-white'}`}>
          ...
          <div className="grid grid-cols-2 gap-2">
            {DESSERTS.map(d => (
              <button key={d.id} onClick={() => setDessert(d.id)}
                className={`border-2 rounded-xl p-2.5 flex items-center gap-2 text-left transition-all hover:-translate-y-1 hover:shadow-md ${dessert === d.id ? 'border-[#c0607a] bg-[#fde8f0] -translate-y-1 shadow-md' : 'border-[#e0d0bc] bg-white'}`}>
          ...
          <div className="grid grid-cols-2 gap-2">
            {SAVOURY.map(s => (
              <button key={s.id} onClick={() => setSavoury(s.id)}
                className={`border-2 rounded-xl p-2.5 flex items-center gap-2 text-left transition-all hover:-translate-y-1 hover:shadow-md ${savoury === s.id ? 'border-[#b86830] bg-[#faecd8] -translate-y-1 shadow-md' : 'border-[#e0d0bc] bg-white'}`}>
      ...
      {/* RIGHT PANEL */}
      <CupPreview base={baseObj} milk={milk} milkRatio={milkRatio} temp={temp} ice={ice} foam={foam} toppings={toppings} />
      <OrderPanel
        base={baseObj} grade={grade} shots={shots} milk={MILKS.find(m=>m.id===milk)} milkRatio={milkRatio}
        temp={temp} ice={ice} size={size} sweetener={sweetener} sweetness={sweetness}
        foam={FOAMS.find(f=>f.id===foam)} toppings={toppings.map(t=>TOPPINGS.find(x=>x.id===t)?.name)}
        dessert={DESSERTS.find(d=>d.id===dessert)} savoury={SAVOURY.find(s=>s.id===savoury)}
